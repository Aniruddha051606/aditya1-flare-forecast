...
def test_global_embargo():
    from solarflare.preprocess.dataset import WindowIndex, chronological_split
    cfg = Config()
    t0 = 1.7e9
    w = [WindowIndex(0, i + 10, t0 + 3600.0 * i) for i in range(24 * 200)]
    tr, va, te = chronological_split(w, cfg, mode="global")
    one_h = min(w[i].t_unix for i in va) - max(w[i].t_unix for i in tr)
    cfg.train.global_embargo_s = 27 * 86400.0
    tr, va, te = chronological_split(w, cfg, mode="global")
    gap_v = min(w[i].t_unix for i in va) - max(w[i].t_unix for i in tr)
    gap_t = min(w[i].t_unix for i in te) - max(w[i].t_unix for i in va)
    check("default gap is the 1 h embargo", one_h <= 2 * 3600.0, f"{one_h}")
    check("27-day embargo between train and validation", gap_v > 27 * 86400.0, f"{gap_v / 86400:.1f} d")
    check("27-day embargo between validation and test", gap_t > 27 * 86400.0, f"{gap_t / 86400:.1f} d")
    s = model_config(_settings(Path(tempfile.gettempdir()), embargo_days=27))
    check("project setting embargo_days reaches the config", s.train.global_embargo_s == 27 * 86400.0)
    per = Config()
    per.train.global_embargo_s = 27 * 86400.0
    _, va_s, _ = chronological_split(w[:48], per, mode="per_segment")
    check("per-segment splits keep the short embargo", len(va_s) > 0)
