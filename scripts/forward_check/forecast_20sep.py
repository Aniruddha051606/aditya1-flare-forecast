"""Sealed day-ahead forecast for 20 Sep 2026 (UTC), issued before its data exist here.
...
DAY0 = datetime(2026, 9, 20, tzinfo=UTC).timestamp()
LEAD0, LEAD1 = 25.0, 49.0                       # hours after an origin at 23:00 UTC two days before
...
def main() -> int:
    ...
    last = int(np.flatnonzero(have & finite & (o <= DAY0 - 3600.0 * LEAD0))[-1])
    origin = o[last]
    result = {}
    for c, lo in (("C", 1e-6), ("M", 1e-5)):
        ...
        print(f">= {c}1 on 20 Sep: P = {p:.3f} ({res['selected']}; test AUC {res['AUC']} {res['AUC_ci']}, "
              f"TSS {res['TSS']}, persistence AUC {pers_auc:.3f}; climatology {clim:.2f}, last 30 d "
              f"{y[recent].mean():.2f})")

    f_last = feat.iloc[last]
    sealed = {
        "forecast_for": "2026-09-20 00:00-24:00 UTC (20 Sep 05:30 IST - 21 Sep 05:30 IST)",
    ...
    (HERE / "forecast_20sep.json").write_text(json.dumps({"sha256": digest, **sealed}, indent=2), encoding="utf-8")
    print("sealed sha256", digest[:16])
    figure(sealed, digest, T, lf, new_events, origin)
    return 0


def figure(s, digest, T, lf, events, origin):
    ...
    t1 = datetime(2026, 9, 21, tzinfo=UTC).timestamp()

    # top: the probabilities, as the headline
    for k, (c, name) in enumerate((("C", "≥C1 flare on 20 Sep"), ("M", "≥M1 flare on 20 Sep"))):
    ...
    ax.text(0, 0.78, "Day-ahead model (SoLEXS activity, logistic /\n"
                     ...
                     "for flares 25–49 h after the last data hour.\n"
                     f"Last data: {s['origin_utc']} UTC. No data for 19 Sep.\n"
                     "GOES for 20 Sep was not opened.\n"
    ...
    d19, d20 = datetime(2026, 9, 19, tzinfo=UTC).timestamp(), DAY0
    ax.axvspan(np.datetime64(int(d19), "s"), np.datetime64(int(d20), "s"), color=GRID, alpha=0.7, lw=0)
    ax.text(np.datetime64(int(d19 + 43200), "s"), -5.45, "19 Sep\nno data\nyet", ha="center", va="center",
    ...
    fig.suptitle("Forecast for 20 September 2026: will the Sun flare?", x=0.07, y=0.975, ha="left",
                 fontsize=15, fontweight="bold")
    fig.text(0.07, 0.905, "Issued before any 20 Sep data were loaded. 20 Sep (UTC) = 05:30 IST on 20 Sep to "
    ...
    out = HERE / "png" / "forecast_2026-09-20_sealed.png"
