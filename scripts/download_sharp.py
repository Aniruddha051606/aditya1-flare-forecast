"""Download SDO/HMI SHARP active-region magnetic parameters (numbers only, no images).
    ...
    pip install drms
    python scripts/download_sharp.py                     # 2024-02 -> this month, into sharp_dir (config)
    python scripts/download_sharp.py --start 2024-02 --end 2026-09 --out E:/AdityaData/sharp
...
downloaded are skipped, except the last ``--refresh-last`` months (default 2):
JSOC publishes with a lag, so a recent month's file starts partial or empty and
must be fetched again later. The script can be stopped and re-run safely. A
month that fails is reported and can be retried.
"""
...
def main() -> int:
    ...
    ap.add_argument("--out", default=str(load_settings().sharp_dir),
                    help="default: sharp_dir in config/project.toml")
    ap.add_argument("--refresh-last", type=int, default=2,
                    help="re-fetch the last N months even if present (JSOC publishes with a lag)")
    ...
    client = drms.Client()
    failed = []
    todo = list(months(args.start, args.end))
    refresh = set(todo[-args.refresh_last:]) if args.refresh_last > 0 else set()
    for y, m in todo:
        dest = out / f"sharp_{y:04d}-{m:02d}.csv"
        if dest.exists() and dest.stat().st_size > 0 and (y, m) not in refresh:
