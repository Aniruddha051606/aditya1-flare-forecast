...
def save(path: Path, job: dict) -> None:
    """Atomic write that tolerates the console holding the file open: on
    Windows os.replace then fails, so retry briefly and fall back to an
    in-place write (same scheme as solarflare.live.write_json_atomic)."""
