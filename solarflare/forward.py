...
def freeze(cfg: Config, checkpoint: Path, name: str, verbose: bool = True) -> Path:
    ...
    # Flux forecast intervals: widen each horizon's spread about the median until
    # the nominal share of VALIDATION outcomes falls inside (split-conformal).
    # Scored like evaluate.py: GOES truth at origin and horizon, SoLEXS observing.
    from .evaluate import solexs_no_change
    from .metrics import forecast_score_mask, interval_scale

    qs = list(cfg.win.quantiles)
    i50 = int(np.argmin(np.abs(np.array(qs) - 0.5)))
    slx_val, _ = solexs_no_change(prep, val["y_t_unix"])
    scales = []
    for h in range(len(cfg.win.forecast_horizons_s)):
        vm_ = (forecast_score_mask(val["y_forecast_mask"][:, h], val["y_nowcast_mask"])
               & (val["y_soft_origin"] > 0) & np.isfinite(slx_val))
        scales.append(round(interval_scale(val["y_forecast"][vm_, h], val["forecast"][vm_, h, 0],
                                           val["forecast"][vm_, h, i50], val["forecast"][vm_, h, -1],
                                           float(qs[-1] - qs[0])), 4))
    calibration["forecast_interval_scale"] = scales
    ...
    manifest = {
        ...
        "forecast_interval_scale": calibration["forecast_interval_scale"],
...
def forward_predict(cfg: Config, name: str, stride_s: float = 60.0,
    ...
    for seg in segments:
        ...
        for b0 in range(0, len(ends), batch_size):
            ...
            if cal and cal.get("forecast_interval_scale"):
                from .metrics import apply_interval_scale
