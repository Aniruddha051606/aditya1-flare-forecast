...
def find_goes_files(goes_dir: Path) -> tuple[list[Path], list[Path]]:
    """Flare summaries (oldest issue first) and 1-minute flux files, one per year.

    NOAA re-issues both as the mission grows (scripts/download_goes.py). Where a
    year's 1-minute file exists in several versions only the highest is read:
    two would put every minute of that year in twice."""
    import re

    d = Path(goes_dir)
    flsum = sorted(d.glob("*xrsf-l2-flsum*.nc"))
    by_year: dict[str, Path] = {}
    for p in sorted(d.glob("*xrsf-l2-avg1m*.nc")):
        m = re.search(r"_y(\d{4})_", p.name)
        key = m.group(1) if m else p.name
        if key not in by_year or p.name > by_year[key].name:
            by_year[key] = p
    return flsum, sorted(by_year.values())
...
def _load_cached(goes_dir: str, stamp: tuple) -> GoesTruth:
    ...
    flares: list[GoesFlare] = []
    seen: set[int] = set()
    for p in reversed(flsum):             # newest issue first: NOAA's revisions win
