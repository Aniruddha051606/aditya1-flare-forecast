...
def make_loaders(prep: Prepared, cfg: Config
    ...
    if getattr(cfg.train, "loader", "gather") == "gather":
        # Batches gathered by index from arrays kept on the device (GatherBatches):
        # the training batch size is unchanged, evaluation uses large batches.
        def gb(key: str, shuffle: bool) -> GatherBatches:
            size = cfg.train.batch_size if shuffle else int(getattr(cfg.train, "eval_batch_size", 1024))
            return GatherBatches(prep.segments, prep.windows, prep.splits[key], cfg, prep._shared,
                                 device, size, shuffle=shuffle,
                                 max_batches=cfg.train.max_batches_per_epoch if shuffle else 0,
                                 seed=cfg.train.seed)

        return gb("train", True), gb("val", False), gb("test", False), stats
