...
Feature sets, all known at t:
  ...
  sharp  SDO/HMI SHARP whole-disk indicators (solarflare.io.sharp) at
         t - LATENCY_S (disk sums and strongest-region maxima), and their 24 h
         change: total and strongest-region unsigned flux, current helicity,
         free energy and R (flux emergence).
...
Writes outputs/dayahead/{DAYAHEAD.md, dayahead_summary.json, dayahead.png} and
frozen_day_models.pkl, the models `python -m solarflare day-forecast` uses.
"""
...
def solexs_minutes(cache_dir) -> tuple[np.ndarray, ...]:
    """SoLEXS minute series from a preprocessing cache: time, rate, counts, valid,
    high-band rate, high-band valid."""
    from solarflare.catalog.build import load_solexs
    from solarflare.catalog.detect import to_minutes

    _, t, r, hi, ok = load_solexs(Path(cache_dir))
    tm, rm, cm, vm = to_minutes(t, r, ok, 20.0)
    _, hm, _, vh = to_minutes(t, hi, ok, 20.0)
    return tm, rm, cm, vm, hm, vh


def fit_calibration(tm, rm, vm, truth, train_end: float):
    """SoLEXS rate -> GOES flux, fitted on training-period minutes only."""
    from solarflare.catalog.detect import PiecewiseCalibration

    gmin = truth.flux_on_grid(tm)
    fit_on = vm & (tm <= train_end) & np.isfinite(gmin) & (gmin > 0) & (rm > 0)
    return PiecewiseCalibration.fit(np.log10(rm[fit_on]), np.log10(gmin[fit_on]))


def flux_series(cal, rm, vm, hm, vh) -> tuple[np.ndarray, np.ndarray]:
    """GOES-scale log flux and background hardness per SoLEXS minute."""
...
def catalogue_flares(path) -> tuple[np.ndarray, np.ndarray]:
    """SoLEXS-detected flares of the master catalogue: when each became known
    (5 min after its peak) and its peak flux, sorted by the former."""
    cat = [r_ for r_ in read_rows(path)
    ...
    return known[order], pf[order]


def window_targets(truth, o: np.ndarray, lead0_h: float, lead1_h: float) -> dict:
    """Per class: did a GOES flare peak in (t + lead0, t + lead1], and did GOES
    observe >= 80% of that window. ``{class: (y, covered)}``."""
    ...
    gt = truth.time_unix
    a, b = np.searchsorted(gt, o + 3600.0 * lead0_h), np.searchsorted(gt, o + 3600.0 * lead1_h)
    covered = (gcs[b] - gcs[a]) / (60.0 * (lead1_h - lead0_h)) >= 0.8
    out = {}
    for c, lo in CLASSES.items():
        pk = np.sort(gp[gf >= lo]) if gp.size else gp
        n = (np.searchsorted(pk, o + 3600.0 * lead1_h, side="right")
             - np.searchsorted(pk, o + 3600.0 * lead0_h, side="right"))
        out[c] = ((n > 0).astype(float), covered)
    return out
...
def xray_features(args):
    ...
    tm, rm, _, vm, hm, vh = solexs_minutes(args.cache_dir)
    cal = fit_calibration(tm, rm, vm, truth, TRAIN_END)
    ...
    return feat, have, o, tgt


def activity_features(tm, lf, hard, known, pf, origins=None):
    """Hourly features known at each origin, from GOES-scale SoLEXS minutes and
    catalogue flares. Returns (features, >= 50% SoLEXS in the last 6 h, origin unix)."""
    ...
    known, pf = np.asarray(known, dtype=np.float64), np.asarray(pf, dtype=np.float64)
...
def sharp_features(sharp_dir: Path, o: np.ndarray):
    ...
    # 24 h change: flux emergence on the whole disk and in the strongest region
    # (the maxima may come from different regions a day apart; that is the point:
    # a new, fast-growing region shows as a jump)
    for k in ("sharp_log_usflux_sum", "sharp_log_totusjh_sum", "sharp_log_usflux_max",
              "sharp_log_totusjh_max", "sharp_log_totpot_max", "sharp_r_value_max"):
        j = sh.names.index(k)
        feat["sharp_d24_" + k.removeprefix("sharp_")] = now[:, j] - day[:, j]
...
#: Frozen day models: one per class and lead, each for "a flare peaks in the 24 h
    ...
    for L0 in DAY_LEADS_H:
        ...
        for c, (y, gok) in window_targets(truth, o, L0, L1).items():
            ...
            if min(tr.sum(), va.sum()) < 50 or len(np.unique(y[tr])) < 2 or len(np.unique(y[va])) < 2:
                continue
            best, best_auc, fitted = None, -1.0, None
            for name, m in _models().items():
                m.fit(X[tr], y[tr])
                a = float(roc_auc_score(y[va], m.predict_proba(X[va])[:, 1]))
                if a > best_auc:
...
def main(argv=None) -> int:
    ...
    for (c, H), (y, gok) in tgt.items():
        ...
        print(f">= {c}1 within {H:2d} h: base rate {res['base_rate_test']:.2f}; AUC {line} | persistence "
              ...
              flush=True)
    # Frozen day models (SoLEXS activity only), so a daily forecast needs no GOES
    # and no refit: python -m solarflare day-forecast --day YYYY-MM-DD
    frozen = freeze_day_models(xf, have, o, truth, cal, data_end=float(tm[-1] + 60.0))
    with open(out / FROZEN_DAY, "wb") as fh:
        pickle.dump(frozen, fh)
    summary["frozen_day_models"] = frozen["skill"]
    print(f"froze {len(frozen['models'])} day models -> {out / FROZEN_DAY}", flush=True)
