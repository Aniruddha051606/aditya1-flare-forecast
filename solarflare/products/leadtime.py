...
NAMES = {"model": "network", "model_no_hel1os": "network, HEL1OS hidden",
         ...
         "hope": "hot onset trigger (HOPE, SoLEXS)"}
    ...
    with torch.no_grad():
        for seg in segments:
            ...
            for b0 in range(0, ends.size, args.batch):
                ...
                if cal and cal.get("forecast_interval_scale"):   # intervals widened on validation
                    from solarflare.metrics import apply_interval_scale
                    i50 = int(np.argmin(np.abs(np.asarray(cfg.win.quantiles) - 0.5)))
                    fc = apply_interval_scale(fc, cal["forecast_interval_scale"], i50)
                rec["fc"].append(fc)
...
#: HOPE trigger on SoLEXS: running difference over this many minutes, and the
#: trigger must hold this many consecutive minutes (Telikicherla et al. 2026,
#: Solar Physics, used 5 consecutive triggers on GOES XRS).
HOPE_LAG_MIN = 5
HOPE_PERSIST_MIN = 5
#: Candidate "hot" cuts on the running-difference hardness, as quantiles of its
#: validation values; the cut is chosen on validation.
HOPE_RATIO_QUANTILES = (0.5, 0.7, 0.8, 0.9, 0.95)


def hope_parts(rm: np.ndarray, vm: np.ndarray, hm: np.ndarray, vh: np.ndarray,
               lag: int = HOPE_LAG_MIN) -> tuple[np.ndarray, np.ndarray]:
    """Running-difference hardness and emission of newly added plasma, per minute.

    The hot onset precursor (Hudson et al. 2021; Telikicherla et al. 2026) is
    plasma already hot (~10 MK) before the impulsive phase, visible as a high
    temperature of the *running difference* of two soft X-ray bands. SoLEXS
    gives a GOES-long-like band (``rm``) and a harder 6-12 keV band (``hm``):
    the ratio of their differences over ``lag`` minutes rises with the
    temperature of the new plasma, the long-band difference with its emission.
    Returns (hardness ratio, emission), NaN where either is undefined."""
    n = rm.size
    ratio, em = np.full(n, np.nan), np.full(n, np.nan)
    if n <= lag:
        return ratio, em
    ok = vm[lag:] & vm[:-lag] & vh[lag:] & vh[:-lag]
    d_l = rm[lag:] - rm[:-lag]
    d_h = hm[lag:] - hm[:-lag]
    up = ok & (d_l > 0) & np.isfinite(d_l) & np.isfinite(d_h)
    with np.errstate(divide="ignore", invalid="ignore"):
        ratio[lag:][up] = d_h[up] / d_l[up]
    em[lag:][up] = d_l[up]
    return ratio, em


def hope_signal(ratio: np.ndarray, em: np.ndarray, cut: float,
                persist: int = HOPE_PERSIST_MIN) -> np.ndarray:
    """Alert score per minute: log10 of the emission while the new plasma is hot
    (ratio >= cut), taken as the minimum over the last ``persist`` minutes so
    the trigger must hold that long; -inf when not hot. Thresholding it on the
    emission reproduces "T > T0 and EM > EM0 for N consecutive minutes"."""
    from numpy.lib.stride_tricks import sliding_window_view

    with np.errstate(divide="ignore", invalid="ignore"):
        s = np.where(np.nan_to_num(ratio, nan=-np.inf) >= cut, np.log10(em), -np.inf)
    s = np.where(np.isfinite(s), s, -np.inf)
    if persist <= 1:
        return s
    pad = np.concatenate([np.full(persist - 1, -np.inf), s])
    return sliding_window_view(pad, persist).min(axis=1)
...
def score(args) -> int:
    ...
    # HOPE hot onset trigger on SoLEXS: the "hot" cut chosen on validation (best
    # minute-level TSS for >= C1), the emission threshold then set like every
    # other method's, at fixed false alarms on validation.
    h_ratio, h_em = hope_parts(rm, vm, hm, vh)
    ratio_g, em_g = g.put(tm + 60.0, h_ratio), g.put(tm + 60.0, h_em)
    yc, yok = labels["C"]
    mv = val & base & yok
    finite_r = ratio_g[mv & np.isfinite(ratio_g)]
    hope_choice = {"cut": None, "val_TSS": None, "candidates": []}
    best_s = np.full(g.n, -np.inf)
    if finite_r.size > 100:
        for q in HOPE_RATIO_QUANTILES:
            cut = float(np.quantile(finite_r, q))
            s_ = hope_signal(ratio_g, em_g, cut)
            fin = np.isfinite(s_)
            tss = tss_threshold(yc[mv], np.where(fin, s_, -99.0)[mv])[1] if fin[mv].any() else -1.0
            hope_choice["candidates"].append({"quantile": q, "cut": round(cut, 5), "val_TSS": round(tss, 3)})
            if hope_choice["val_TSS"] is None or tss > hope_choice["val_TSS"]:
                hope_choice.update(cut=round(cut, 5), val_TSS=round(tss, 3))
                best_s = s_
    hope = np.where(np.isfinite(best_s), best_s, np.nan)
    hope[base & ~np.isfinite(hope)] = -99.0          # observed but not triggered: never ON
    sig["hope"] = {"C": hope, "M": hope}
    ...
    methods = {"C": ["model", "trend", "current", "hope", "rule", "rule_C_level"],
               "M": ["model", "combined", "trend", "current", "hope"]}
    ...
    summary: dict = {
        ...
        "hope_trigger": {"lag_min": HOPE_LAG_MIN, "persist_min": HOPE_PERSIST_MIN, **hope_choice},
    }
    lead_cols: dict = {}
    for c in ("C", "M"):
...
def write_watch(path: Path, g, flux, goes, sig: dict, hard_frac, flares, test_start: float,
    ...
    the end of their minute, GOES likewise (its record [A, A+60) at A+60)."""
    f32 = np.float32
