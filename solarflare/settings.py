...
def load_settings(path: str | None = None, root: str | None = None) -> Settings:
    ...
    p = raw.get("paths", {})
    # SOLARFLARE_DATA_ROOT moves every data folder at once (e.g. to a new drive);
    # the other data paths may say "{data_root}/..." to follow it.
    data_root = _resolve(os.environ.get("SOLARFLARE_DATA_ROOT") or p.get("data_root", "data"), root)

    def under(key: str, default: str) -> Path:
        return _resolve(str(p.get(key, default)).replace("{data_root}", data_root.as_posix()), root)

    return Settings(
        root=root,
        data_root=data_root,
        goes_dir=under("goes_dir", "{data_root}/goes"),
        sharp_dir=under("sharp_dir", "{data_root}/sharp"),
        hel1os_extracted=under("hel1os_extracted", "{data_root}/extracted/hel1os"),
...
def model_config(s: Settings, out_dir: Path | None = None, sharp: bool | None = None):
    ...
    cfg.train.global_embargo_s = float(m.get("embargo_days", 0.0)) * 86400.0
