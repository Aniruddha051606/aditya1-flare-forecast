...
def sec_data(s: Settings) -> list[str]:
    ...
    pre = cfg.get("pre", {})
    tr = cfg.get("train", {})
    emb = max(float(tr.get("embargo_s", 3600.0)), float(tr.get("global_embargo_s", 0.0)))
    gap = f"{emb / 86400:g}-day" if emb >= 86400 else f"{emb / 3600:g} h"
    L = [f"Chronological split: training to {d(sp['train_end'])}, test from {d(sp['test_start'])} "
         f"({gap} embargo between splits). Flare truth: GOES-18 XRS flare list and flux, >= "
...
def sec_network(s: Settings) -> list[str]:
    ...
    def ci(x, key):
        c = x.get(f"{key}_ci95")
        return f"{_f(x.get(key))} [{_f(c[0])}, {_f(c[1])}]" if c else _f(x.get(key))

    L = [f"Best epoch {ev['training']['best_epoch']} (validation score {ev['training']['best_score']:.3f}, "
         "smoothed over epochs). Brackets: 95% intervals resampling whole test days.", "",
         "| task (test split) | TSS | POD | FAR | AUC | Brier skill |", "|---|---|---|---|---|---|",
         f"| flare in progress now | {ci(nf, 'TSS')} | {ci(nf, 'POD')} | {_f(nf['FAR'])} | {ci(nf, 'AUC')} | "
         f"{_f(nf.get('BSS_vs_climatology'))} |"]
    for h, x in ev.get("forecast_occurrence", {}).items():
        if isinstance(x, dict) and "TSS" in x:
            L.append(f"| flare within {h} | {ci(x, 'TSS')} | {_f(x['POD'])} | {_f(x['FAR'])} | {ci(x, 'AUC')} | "
                     f"{_f(x.get('BSS_vs_climatology'))} |")
    nr = ev.get("nowcast_regression", {})
    fr = ev.get("forecast_regression", {})
    L += ["", f"GOES flux now from Aditya data (where SoLEXS observed): MAE {ci(nr, 'MAE')} dex"
          + (f"; a one-line SoLEXS calibration on the same windows: {_f(nr['solexs_calibration_MAE'])} dex"
             if "solexs_calibration_MAE" in nr else "") + "."]
    rows = [(h, x) for h, x in fr.items() if isinstance(x, dict) and "MAE" in x]
    if rows:
        L += ["", "| flux forecast | MAE (dex) | RMSE | vs no change (GOES) | vs no change (SoLEXS) | "
                  "80% range covers | after widening |", "|---|---|---|---|---|---|---|"]
        for h, x in rows:
            L.append(f"| +{h} | {ci(x, 'MAE')} | {_f(x.get('RMSE'))} | {_f(x.get('skill_vs_persistence'))} | "
                     f"{_f(x.get('skill_vs_solexs_no_change'))} | {_f(x.get('interval_coverage'), 2)} | "
                     f"{_f(x.get('interval_coverage_calibrated'), 2)} |")
        L.append("Skill > 0 beats that 'no change' reference. GOES no-change reads the instrument being "
                 "predicted; SoLEXS no-change is what an Aditya-only system has. Ranges are widened by a "
                 "factor fitted on validation.")
    pk = ev.get("peak")
    if pk:
        L.append(f"Peak of flares under way: size MAE {ci(pk, 'log_peak_flux_MAE')} dex, time MAE "
