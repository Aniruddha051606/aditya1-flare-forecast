...
class TrainConfig:
    ...
    #: Gap between the blocks of a *global* (calendar) split, seconds; the larger
    #: of this and ``embargo_s`` applies. An active region lives for weeks and
    #: returns after a solar rotation, so with a 1 h gap the same regions flare
    #: at the end of validation and the start of test. 0 reproduces runs up to
    #: 2026-09; project.toml sets one rotation (27 d). Per-segment splits (short
    #: samples) keep ``embargo_s``: a 27-day gap would empty them.
    global_embargo_s: float = 0.0
    ...
    #: "gather" builds whole batches by tensor indexing from arrays held on the
    #: device (torch_data.GatherBatches); "torch" is the per-window DataLoader
    #: used up to the 2026-09 run, where Python slicing, not the GPU, set the pace.
    loader: str = "gather"
    #: Batch size for validation and test passes (no gradients, so it only sets
    #: speed and memory; results are identical).
