...
def hel1os_minutes(cache: Path) -> dict[str, tuple[np.ndarray, np.ndarray]]:
    """HEL1OS minute light curves for display: CdTe 5-20 keV and CZT 20-40 keV,
    each the sum of its two detectors over minutes both of them observed
    (counts/s). Returns ``{band: (minute start, rate)}``."""
    entries = json.loads((cache / "manifest.json").read_text("utf-8"))
    bands = (("cdte_5_20", "hls_cdte{}_5_20keV", "hls_cdte{}_cov"),
             ("czt_20_40", "hls_czt{}_20_40keV", "hls_czt{}_cov"))
    acc: dict[str, tuple[list, list]] = {k: ([], []) for k, _, _ in bands}
    for h in stitch(load_cached(entries, cache, "hel1os"), DT, 21600.0):
        names = list(h.names)
        for key, rate, cov in bands:
            r = h.values[:, names.index(rate.format(1))] + h.values[:, names.index(rate.format(2))]
            ok = ((h.values[:, names.index(cov.format(1))] > 0) & (h.values[:, names.index(cov.format(2))] > 0)
                  & np.isfinite(r))
            tm, rm, _, vm = to_minutes(h.time_unix, r, ok, DT)
            acc[key][0].append(tm[vm])
            acc[key][1].append(rm[vm])
    return {k: (np.concatenate(t) if t else np.zeros(0), np.concatenate(v) if v else np.zeros(0))
