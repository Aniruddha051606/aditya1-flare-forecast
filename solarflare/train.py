...
def collect_predictions(model: SolexHelNet, loader: DataLoader,
    ...
    for batch in loader:
        ...
        for k in ("in_flare", "phase", "nowcast", "nowcast_mask", "forecast",
            ...
            push(f"y_{k}", batch[k].cpu().numpy())
        # Which instruments observed the forecast origin: an Aditya-only system
        # forecasts flux only where SoLEXS is observing (see evaluate.py).
        push("y_soft_origin", batch["soft_mask"][:, -1].cpu().numpy())
        push("y_hard_origin", batch["hard_mask"][:, -1].cpu().numpy())
