...
class DataMissing(RuntimeError):
    """The data folder is empty, or has lost files this cache was built from."""


#: Fraction of previously cached source files that may disappear (e.g. an old
#: version tidied away) before the cache refuses to carry on.
...
def check_sources_present(sources: list[Source], old_entries: list[dict],
    ...
    if prev and len(gone) > MAX_MISSING_FRACTION * len(prev):
        raise DataMissing(f"{len(gone)} of {len(prev)} files this cache was built from are missing "
                          f"(e.g. {gone[0]}). Restore them or point data_root at their new "
                          "location; set SOLARFLARE_ALLOW_MISSING=1 to continue without them.")
