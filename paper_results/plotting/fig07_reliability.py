...
def main() -> int:
    ...
    for ax, h in zip(axes, PANELS):
        ax.plot([0, 1], [0, 1], color="0.6", lw=0.6, ls=":", label="perfect reliability")
        for kind, st in (("raw", {"color": "0.5", "ls": "--", "marker": "s", "mfc": "white"}),
                         ("calibrated", {"color": "0.0", "ls": "-", "marker": "o", "mfc": "0.0"})):
    ...
    cap = ("Reliability of the E4 probabilities for >= C1 flares on the test period (all E4 test windows), before "
           "(open squares) and after (filled circles) isotonic calibration fitted on the validation period only. "
