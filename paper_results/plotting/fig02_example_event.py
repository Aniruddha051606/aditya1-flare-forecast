...
SOFT = [("slx_2_3keV", "2-3 keV", "-"), ("slx_4_6keV", "4-6 keV", "--"), ("slx_8_12keV", "8-12 keV", ":")]
HARD = [("20_40keV", "20-40 keV", "-"), ("40_60keV", "40-60 keV", "--"), ("60_80keV", "60-80 keV", ":")]
...
def main() -> int:
    ...
    ax = axes[0]
    ax.semilogy(mins(g.time_unix[m]), g.xrsb[m], color="0.0", lw=1.0, label="GOES-18 XRS-B (0.1-0.8 nm)")
    ...
    ax = axes[1]
    for key, label, ls in SOFT:
        ax.semilogy(mins(ts), soft[key], color="0.0", ls=ls, lw=0.9, label=f"SoLEXS {label}")
    ax.set_ylabel("Count rate (s$^{-1}$)")
    ax.legend(loc="upper right")
    ax = axes[2]
    for key, label, ls in HARD:
        ...
        ax.semilogy(mins(th), np.where(rate > 0, rate, np.nan), color="0.0", ls=ls, lw=0.9,
